                    ...
                    <tr key={u._id} className="hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl border border-border hover:bg-white/[0.03] text-muted-foreground hover:text-foreground transition font-medium cursor-pointer min-h-[36px]"
                        >
                          Toggle Admin
                        </button>
                        <select
                          ...
                          className="rounded-xl bg-input border border-border px-2.5 py-1.5 outline-none focus:ring-1 focus:ring-primary/40 text-foreground cursor-pointer min-h-[36px]"
                        >
          ...
          <div className="space-y-4">
            {messages.length === 0 ? (
              ...
              messages.map((m) => (
                <div
                  key={m._id}
                  className="glass rounded-2xl p-5 border border-border/40 flex flex-col sm:flex-row justify-between gap-4 sm:gap-6"
                >
                  <div className="space-y-3 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed p-3.5 bg-black/15 rounded-xl border border-border/30 break-words">
                      {m.message}
                    </p>
                  </div>
                  <div className="shrink-0 flex items-start self-end sm:self-start">
                    <button
                      onClick={() => deleteMessage(m._id)}
                      className="p-2.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-xl transition cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center"
