                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="hover:text-destructive p-1 rounded transition cursor-pointer min-h-[32px] min-w-[32px] flex items-center justify-center"
          ...
          <div className="glass rounded-2xl p-5 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full accent-primary bg-input rounded-lg h-2 cursor-pointer"
              />
            </div>
            ...
            <button
              ...
              className="w-full rounded-xl bg-gradient-to-r from-primary to-accent px-4 py-3.5 text-xs font-semibold text-primary-foreground flex items-center justify-center gap-2 hover:scale-[1.01] transition shadow-lg cursor-pointer disabled:cursor-not-allowed min-h-[44px]"
            >
        ...
        <div className="lg:col-span-3 space-y-6">
          {slides.length > 0 ? (
            <div className="space-y-6">
              {/* Slide Screen */}
              <div className="relative aspect-[16/9] min-h-[280px] w-full glass rounded-3xl p-5 sm:p-8 bg-card/90 flex flex-col justify-between overflow-hidden shadow-2xl border border-border/50">
                ...
                <div className="flex-1 flex flex-col justify-center my-4 sm:my-6 overflow-y-auto">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <h2 className="text-lg sm:text-2xl font-bold font-display leading-tight gradient-text">
                        {currentSlide.title}
                      </h2>
                      <ul className="space-y-2">
                        {currentSlide.content?.map((bullet, i) => (
                          <li key={i} className="text-xs sm:text-sm text-muted-foreground flex items-start gap-2.5">
                            <span className="h-1.5 w-1.5 rounded-full bg-accent mt-1.5 shrink-0 animate-pulse-glow" />
                            <span>{bullet}</span>
                          </li>
                        ))}
                      </ul>
                ...
                <div className="flex justify-between items-center shrink-0 select-none border-t border-border/30 pt-3">
                  <span className="text-[10px] text-muted-foreground truncate max-w-[60%] sm:max-w-[70%]">{topic}</span>
                  <div className="flex gap-2">
                    <button
                      onClick={prevSlide}
                      disabled={currentSlideIndex === 0}
                      className="h-9 w-9 rounded-xl bg-white/[0.03] border border-border/60 hover:bg-white/[0.08] disabled:opacity-30 grid place-items-center transition cursor-pointer min-h-[36px] min-w-[36px]"
                    >
